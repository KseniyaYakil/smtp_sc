...
const char *smtp_cmd_arr[SMTP_CMD_LAST] = {
	[SMTP_CMD_HELO] = "HELO",
	[SMTP_CMD_EHLO] = "EHLO",
	[SMTP_CMD_MAIL] = "MAIL",
	[SMTP_CMD_DATA] = "DATA",
...
static struct smtp_reg *smtp_reg_arr[SMTP_CMD_LAST] = {
	[SMTP_CMD_HELO] = {
		.str = "("DOMAIN")",
	},
...
static void smtp_data_internal_init(void)
{
	for (uint32_t i = 0; i < SMTP_CMD_LAST; i++) {
		struct smtp_reg *r = &smtp_reg_arr[i];
		const char *err;
		int err_off;
...
static char *cmd_parse(pcre *re, const char *data, int data_len, int *len) {
	int ovec[24];
	...
	*len = ovec[1] - ovec[0];
	if (rc < 0) {
		slog_e("Invalid command came, data == '%.*s'\n", (int)data_len, data);
		*len = -1;
		return NULL;
	}
...
static enum smtp_cmd determine_cmd(struct buf *buf)
{
	...
	for (uint32_t i = 0; i < SMTP_CMD_LAST; i++) {
		if (strncmp(smtp_cmd_arr[i], cmd, sizeof(smtp_cmd_arr[i])) == 0) {
			smtp_cmd = i;	
			break;
		}
	}
...
{
	enum smtp_cmd smtp_smd = determine_cmd(msg);
	...
	case SMTP_CMD_VRFY:
	case SMTP_CMD_QUIT: {
		if (smtp_cmd_arr[smtp_cmd].re == NULL)
			break;

		const char *args_str = buf_get_data(msg) +
				       sizeof(smtp_cmd_arr[smtp_cmd].str);
		uint32_t len = buf_get_len(msg) -
			       sizeof(smtp_cmd_arr[smtp_cmd].str);
		const char *args = cmd_parse(smtp_cmd[smtp_cmd].re, args_str, len, &len);
		if (args == NULL) {
	...
	if (ans->ret_msg_len == -1) {
		slog_e("smtp_data: err in snprintf");
		return -1;
	}
